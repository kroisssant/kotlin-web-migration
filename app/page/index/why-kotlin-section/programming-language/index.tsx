...
export function ProgrammingLanguage() {
    ...
    const [highlighted, setHighlighted] = useState(
        () => hljs.highlight('kotlin', tabs[0].code).value
    );

    const highlight = useCallback((index: number) => {
        setHighlighted(hljs.highlight('kotlin', tabs[index].code).value);
    }, []);
    ...
    return (
        <div className="kto-grid kto-grid-gap-32 kto-offset-top-96 kto-offset-top-md-48">
            ...
            <div className="kto-col-8 kto-col-md-12">
                <TabList value={activeIndex} onChange={(v: number) => handleTabChange(v)}>
                    {tabs.map((tab, i) => (
